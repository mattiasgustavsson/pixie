 ...
 */
int jar_xm_generate_samples(jar_xm_context_t*, float* output, size_t numsamples);
// MG: added return value for samples generated
...
// MG: new create func to avoid malloc
int jar_xm_create_context_mempool(jar_xm_context_t** ctxp, const char* moddata, size_t moddata_length, uint32_t rate, 
    char* mempool, size_t capacity );

// MG: moved to header for use with new mempool version of create
...
// MG: added function to avoid alloc
int jar_xm_create_context_mempool(jar_xm_context_t** ctxp, const char* moddata, size_t moddata_length, uint32_t rate, char* mempool, size_t capacity ) {
#if JAR_XM_DEFENSIVE
    int ret;
#endif
    size_t bytes_needed;
    ...
    if(mempool == NULL || bytes_needed > capacity) {
        /* malloc() failed, trouble ahead */
        DEBUG("mempool too small");
        return 2;
    }
...
// MG: refactor to use mempool func
int jar_xm_create_context_safe(jar_xm_context_t** ctxp, const char* moddata, size_t moddata_length, uint32_t rate) {
    ...
    *ctxp = 0;
    int result = jar_xm_create_context_mempool( ctxp, moddata, moddata_length, rate, mempool, bytes_needed );
    if( *ctxp )
    {
        (*ctxp)->allocated_memory = mempool; /* Keep original pointer for free() */
        if( result != 0 ) jar_xm_free_context(*ctxp);
    }
    return result;
}

void jar_xm_free_context(jar_xm_context_t* context) {
    if( context->allocated_memory ) free(context->allocated_memory);
}
...
static int jar_xm_sample(jar_xm_context_t*, float*, float*);
...
// MG: return value for when max loop is reached
static int jar_xm_sample(jar_xm_context_t* ctx, float* left, float* right) {
...
// mg: return value for samples produced
int jar_xm_generate_samples(jar_xm_context_t* ctx, float* output, size_t numsamples) {
    if(ctx && output) {
        ctx->generated_samples += numsamples;
        for(size_t i = 0; i < numsamples; i++) {
            if( !jar_xm_sample(ctx, output + (2 * i), output + (2 * i + 1)) )
                return (int)i;
        }
    }
